...
public class EasyTabBarTxtScroll extends HorizontalScrollView {
    ...
    private void initAttrs(AttributeSet attrs) {
        ...
        mHeight = ta.getLayoutDimension(R.styleable.EasyTabBarTxtScroll_android_layout_height, 0);
        mWidth = ta.getLayoutDimension(R.styleable.EasyTabBarTxtScroll_android_layout_width, 0);

        mIndicatorColor = ta.getColor(R.styleable.EasyTabBarTxtScroll_indicatorColor, INDICATOR_COLOR_DEFAULT);
        mIndicatorHeight = ta.getDimension(R.styleable.EasyTabBarTxtScroll_indicatorHeight, 0);
        mIndicatorWidth = ta.getDimension(R.styleable.EasyTabBarTxtScroll_indicatorWidth, 0);
        mIndicatorCornerRadius = ta.getDimension(R.styleable.EasyTabBarTxtScroll_indicatorCornerRadius, -1);
        mIndicatorType = ta.getInt(R.styleable.EasyTabBarTxtScroll_indicatorType, INDICATOR_TYPE_LINE);
        mIndicatorFit = ta.getInt(R.styleable.EasyTabBarTxtScroll_indicatorFit, INDICATOR_FIT_WIDTH);
        mIndicatorPaddingLeft = (int) ta.getDimension(R.styleable.EasyTabBarTxtScroll_indicatorPaddingLeft, 0);
        mIndicatorPaddingRight = (int) ta.getDimension(R.styleable.EasyTabBarTxtScroll_indicatorPaddingRight, 0);

        mUnderlineColor = ta.getColor(R.styleable.EasyTabBarTxtScroll_underlineColor, COLOR_DEFAULT);
        mUnderlineHeight = ta.getDimension(R.styleable.EasyTabBarTxtScroll_underlineHeight, 0);

        mTabTextsize = ta.getDimension(R.styleable.EasyTabBarTxtScroll_tabTextSize, sp2px(mContext, TEXT_SIZE_DEFAULT));
        mTabWidth = (int) ta.getDimension(R.styleable.EasyTabBarTxtScroll_tabWidth, -2f);//-2为WRAP_CONTENT属性
        mTabHeith = (int) ta.getDimension(R.styleable.EasyTabBarTxtScroll_tabHeight, -2f);
        mTabPaddingLeft = (int) ta.getDimension(R.styleable.EasyTabBarTxtScroll_tabPaddingLeft, 0);
        mTabPaddingRight = (int) ta.getDimension(R.styleable.EasyTabBarTxtScroll_tabPaddingRight, 0);
        mTabPaddingTop = (int) ta.getDimension(R.styleable.EasyTabBarTxtScroll_tabPaddingTop, 0);
        mTabPaddingBottom = (int) ta.getDimension(R.styleable.EasyTabBarTxtScroll_tabPaddingBottom, 0);
        mTabSelectTextColor = ta.getColor(R.styleable.EasyTabBarTxtScroll_tabSelectTextColor, TEXT_COLOR_SELECT_DEFAULT);
        mTabUnSelectTextColor = ta.getColor(R.styleable.EasyTabBarTxtScroll_tabUnSelectTextColor, TEXT_COLOR_UNSELECT_DEFAULT);
        mTabTextBold = ta.getBoolean(R.styleable.EasyTabBarTxtScroll_tabTextBold, false);
        mTabTextItalic = ta.getBoolean(R.styleable.EasyTabBarTxtScroll_tabTextItalic, false);
        mTabWeight = ta.getBoolean(R.styleable.EasyTabBarTxtScroll_tabWeight, false);
...
//            EasyUILog.e("EasyTabBarTxtScroll setViewPager titles is null");
    ...
    private void initTabViews() {
        ...
        for (int i = 0; i < mTabCount; i++) {
            ...
            textView.getPaint().setFakeBoldText(i == mCurrentTab);
...
//            LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.WRAP_CONTENT, LinearLayout.LayoutParams.WRAP_CONTENT);
    ...
    private void updtaeTabText(int position) {
        for (int i = 0; i < mTabCount; i++) {
            ...
            textView.getPaint().setFakeBoldText(i == position);
        }
    }
     ...
     * @param tabText
     */
    public void updtaeTabText(int position,SpannableString tabText) {
    ...
    private void scrollTabToCenter() {
        ...
        int x = 0;
        if(null != newTv){
            x = (int) (left + newTv.getWidth() / 2 - mWidth / 2);
        }
        scrollTo(x, 0);
    }
...
//            newTv.getX() > scrollX ||
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mWidth <= 0) {
//            mWidth = getWidth();
            //使用getWith会导致在实际使用时，拿到的宽度和实际展示的宽度不一致的情况
            //具体表现会tab不跟随viewpager滚动，因此换成getMeasuredWidth
...
//                        + rightOffset * mPositionOffset + mIndicatorPaddingRight;
                ...
                left = textView.getX()+ textView.getWidth() / 2 - txtWidth / 2
                        + lfetOffset * mPositionOffset;
                right = textView.getX() + textView.getWidth() / 2 + txtWidth / 2
                        + rightOffset * mPositionOffset;
            } else {
